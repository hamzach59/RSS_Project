#  RSS Feed Filter
# Name:MUHAMMAD HAMZA,HAMZA SAIF GONDAL,AHMED SULEMAN,FAWAD KARIM
# Collaborators:
# Time:
    ...
    for entry in entries:
        ...
        try:
            ...
            try:
                ...
                pubdate = datetime.now(pytz.timezone("GMT"))
...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self, text):
        ...
        for i in range(len(word_list) - len(phrase_split) + 1):
            if phrase_split == word_list[i : i + len(phrase_split)]:
                return True
        return False
...
class BeforeTrigger(TimeTrigger):
    def evaluate(self, story):
        return self.pubtime > story.get_pubdate()
...
def read_trigger_config(filename):
    ...
    for i in range(len(lines)):
        ...
        if trig[1] == "TITLE":
            ...
            trig_dict[trig[0]] = NotTrigger(trig_dict[trig[2]])
        elif trig[1] == "AND":
